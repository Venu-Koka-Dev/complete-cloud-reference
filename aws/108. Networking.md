...
Note: Size of an existing CIDR block cannot be increased or decreased; it is locked after creation
Note: The primary IPv4 CIDR block that you choose for your VPC will determine the number and size of IPv4 addresses that can be assigned to the subnets created within the VPC
Note: (Optional) VPC supports IPv6 CIDR blocks also 
Note: CIDR blocks must not overlap with any existing CIDR blocks associated within a VPC or with another VPC connected with a peering connection
Note: The initial CIDR block that was added when you created the VPC can’t be changed; however, you have the option of adding additional secondary CIDR blocks to an existing VPC
